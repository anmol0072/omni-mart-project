...
interface OrderItem { id: string; quantity: number; products?: { name: string; images: string[] } }
interface Order { id: string; created_at: string; total: number; status: string; tracking_id?: string; courier_name?: string; estimated_delivery?: string; sellers?: { business_name: string }; order_items?: OrderItem[] }
interface Favorite { id: string; product_id: string; products?: { name: string; base_price: number; unit: string } }
interface Address { id: string; label: string; is_default: boolean; full_name: string; address_line1: string; address_line2?: string; city: string; state: string; pincode: string; phone?: string }
