...
import { Package, ShoppingBag, IndianRupee, Plus, Edit, Trash2, CheckCircle, XCircle, Truck } from "lucide-react";
...
interface OrderItem { id: string; quantity: number; products?: { name: string } | null }
interface Profile { id: string; full_name?: string | null }
type Order = Database["public"]["Tables"]["orders"]["Row"] & { order_items?: OrderItem[]; profiles?: Profile | null };
...
export default function SellerDashboard() {
  ...
  const editProduct = (p: Product) => {
    ...
    setProductForm({
      ...
      base_price: p.base_price, discount_percent: p.discount_percent || 0, status: p.status as "draft" | "active" | "out_of_stock",
      images: p.images || [],
    });
  ...
  const updateOrderStatus = async (orderId: string, status: string) => {
    await supabase.from("orders").update({ status: status as "placed" | "confirmed" | "processing" | "shipped" | "out_for_delivery" | "delivered" | "cancelled" | "rejected" }).eq("id", orderId);
                ...
                <DialogContent className="max-h-[80vh] overflow-y-auto">
                  ...
                  <div className="space-y-3">
                    ...
                    <div><Label>Status</Label>
                      <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={productForm.status} onChange={e => setProductForm(f => ({ ...f, status: e.target.value as "draft" | "active" | "out_of_stock" }))}>
