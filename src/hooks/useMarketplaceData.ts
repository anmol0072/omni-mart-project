...
function mapDbProduct(p: any): Product {
  ...
  return {
    ...
    pricingTiers: p.pricing_tiers?.length
      ? p.pricing_tiers
          .sort((a: { min_qty: number | string }, b: { min_qty: number | string }) => Number(a.min_qty) - Number(b.min_qty))
          .map((t: { min_qty: number | string; max_qty: number | string | null; price_per_unit: number | string }) => ({
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Seller[]> => {
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("status", "approved");
      if (error || !data || data.length === 0) return mockSellers;
      return data.map(mapDbSeller);
    },
    staleTime: 1000 * 60 * 5,
  });
}
